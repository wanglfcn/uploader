...
class OutputLooper(object):
    def __init__(self, chan, attr, stream, capture, timeout, cmd=''):
        self.chan = chan
        self.stream = stream
    ...
    def loop(self):
        ...
        while True:
            ...
            try:
                ...
                if self.attr == 'err':
                    pass
                elif len(bytelist) > 0 and afterInit == 0:
                    promote = all_contents.splitlines()[-1]
                    if not promote.startswith('##'):
                        afterInit = 1
                        self.chan.sendall(self.cmd + '\n')

                elif len(bytelist) > 0 and afterInit == 1:
                    if all_contents.splitlines()[-1] == promote:
                        self.chan.sendall('exit\n')
